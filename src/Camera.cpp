...
Camera::Camera(World& world, Point2D position, double vPos, double height, double health, std::string texture, double fieldOfView, double eyesHeight, double depth, double walkSpeed, double jumpSpeed, double viewSpeed)
    : W_world(world), Player(position, vPos, height, health, std::move(texture)), d_eyesHeight(eyesHeight), d_depth(depth), d_walkSpeed(walkSpeed), d_jumpSpeed(jumpSpeed), d_viewSpeed(viewSpeed)
{
    Weapon weapon1(300);
...
void Camera::addPlayer(const std::string& name, std::shared_ptr<Player> camera)
{
...
void Camera::objectsRayCrossed(const pair<Point2D, Point2D>& ray, std::vector<RayCastStructure>& v_rayCastStruct, const std::string& name, int reflections)
{
    ...
    {
        if (object.first == name || object.second == nullptr || object.second->nodes().size() < 2)
            continue;
        ...
        if (object.second->cross(ray, wall, crossPoint, len))
        {
            ...
            v_rayCastStruct.push_back({ (ray.first - crossPoint).abs(), len, object.second.get(), object.second->height(), v_reflectedRayCastStructure });
            // for collision
    ...
    // collision
    if (W_world[nearObject] != nullptr && b_collision && W_world[nearObject] != nullptr && name == getName() && COLLISION_AREA >= closest)
    {
    ...
    // Bonus collision
    if (W_world[nearObject] != nullptr && !nearObject.empty() && name == getName() && (W_world[nearObject]->position() - position()).abs() <= COLLISION_AREA && W_world[nearObject]->type() == ObjectType::Bonus)
    {
        bonusM.lock();
...
void Camera::draw(sf::RenderTarget& window)
{
    ...
    circle.setPosition((double)x() * SCALE - COLLISION_DISTANCE * SCALE, (double)y() * SCALE - COLLISION_DISTANCE * SCALE);
