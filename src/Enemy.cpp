...
void Enemy::connect(const std::string& clientIp, sf::Uint16 clientPort) {
...
void Enemy::objectsRayCrossed(const pair<Point2D, Point2D>& ray, std::vector<RayCastStructure>& v_rayCastStruct)
{
    ...
    {
        if (object.first == getName() || object.second == nullptr || object.second->nodes().size() < 2)
            continue;
...
void Enemy::hiddenObjectsRayCrossed(const pair<Point2D, Point2D>& ray, const std::string& name)
{
    ...
    for (auto& object : W_world.objects())
    {
        if (object.second == nullptr || object.first == name || object.second->nodes().size() < 2)
            continue;
...
double Enemy::angleBetween(const std::string& obj) const {
