...
void GeneticAlgorithm::saveNetwork(std::string filename) {
    ...
    v_generation[maxKillsPlayer]->network().saveNetwork(std::move(filename), i_generation);
}

void GeneticAlgorithm::loadNetwork(const std::string& filename) {
...
void GeneticAlgorithm::logScore(const std::string& filename) {
    std::ofstream file;
