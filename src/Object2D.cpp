...
Object2D::Object2D(Point2D position, std::vector<Point2D> points, double height, const std::string& texture, Point2D velocity)
    : p_position(position), v_points2D(std::move(std::move(points))), p_velocity(velocity), d_height(height), T_texture(*ResourceManager::loadTexture(texture))
{
}
