...
std::shared_ptr<Object2D> World::findObject2D(const std::string& name) const
{
    return map_objects[name];
}
...
std::shared_ptr<Object2D> World::operator[](const std::string& name) const
{
    return findObject2D(name);
}
...
bool World::load3DObj(const std::string& filename, const std::string& texture, double scale, Point2D position) {
    ...
    for (size_t i = 0; i < indices.size(); i++) {
        ...
        auto* wall = new Poligon2D({{vertices[indices[i].z].x * scale, vertices[indices[i].z].z * scale}, {vertices[indices[i].x].x * scale, vertices[indices[i].x].z * scale}}, position, vertices[indices[i].y].y * scale, texture);

        for(const auto& obj : map_objects)
            for (auto node1 : obj.second->nodes())
                if (node1 == wall->nodes()[0])
                    for (auto node2 : obj.second->nodes())
