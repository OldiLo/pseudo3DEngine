...
public:
    explicit World(double length = 100, double width = 100, const std::string& sky_texture = SKY_TEXTURE, const std::string& floor_texture = FLOOR_TEXTURE);
    ...
    std::shared_ptr<Object2D> findObject2D(const std::string& name) const;

    std::shared_ptr<Object2D> operator[](const std::string& name);
    std::shared_ptr<Object2D> operator[](const std::string& name) const;
    ...
    bool load3DObj(const std::string& filename, const std::string& texture = FLOOR_TEXTURE, double scale = 1.0, Point2D position = {0, 0});
